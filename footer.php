<!--Pie de pagina-->
<div class="footer-smedia-bc">
  <ul class=" footer-smedia">
    <li><img class="footer-img"src="./images/Facebook.png" alt=""></li>
    <li><img class="footer-img"src="./images/Instagram.png" alt=""></li>
    <li><img class="footer-img"src="./images/Pinterest.png" alt=""></li>
    <li><img class="footer-img"src="./images/Twitter.png" alt=""></li>
    <li><img class="footer-img"src="./images/YouTube.png" alt=""></li>
  </ul>
</div>
<div class="footer-media-bc">
  <ul class=" footer-smedia">
    <li><img class="footer-img"src="./images/Facebook.png" alt="">Facebook</li>
    <li><img class="footer-img"src="./images/Instagram.png" alt="">Instagram</li>
    <li><img class="footer-img"src="./images/Pinterest.png" alt="">Pinterest</li>
    <li><img class="footer-img"src="./images/Twitter.png" alt="">Twitter</li>
    <li><img class="footer-img"src="./images/YouTube.png" alt="">YouTube</li>
  </ul>
</div>
<footer class="fuente-pie pie_pagina">
  <div class="footer">
    <h5>ACERDA DE FID</h5>
    <br>
    <ul>
      <li>Termino y condiciones</li>
      <li>Politica de Privacidad</li>
      <li>Guia de talles</li>
      <li>Costos y tiempos de entrega</li>
      <li>Cambios y devoluciones</li>
    </ul>
  </div>
  <div class="footer">
    <h5>CONTACTO</h5>
    <br>
    <ul>
      <li><a style="color: #fff; 	font-size: 15px" href="contacto.php"><p class="fuente-pie">Formulario de Contacto</p></a></li>
    </ul>
  </div>
  <div class="footer">
    <ul>
      <li>DIF S.A.</li>
      <br>
      <li>consultas@dif.com.ar</li>
      <li>0800-222-3344</li>
    </ul>
  </div>
  </footer>
  <div class="copyright">
    <p>Copyright @ DIF 2018</p>
</div>
